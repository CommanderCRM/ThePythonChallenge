# nonogram solver, credit to hackingnote
...
class Seg:
  ...
  def __repr__(self):
    return '<Seg:len=' + str(self.length) + ',placements=' + str(
      self.placements) + '>'
...
class Bar:
  ...
  def __init__(self, axis, index, segs, length):
    ...
    self.index = index
    self.length = length
    self.full_space = sum([seg.length
                           for seg in self.segs]) + len(self.segs) - 1
  ...
  def __repr__(self):
    return '<Bar:axis=' + str(self.axis) + ',index=' + str(
      self.index) + ',len=' + str(self.length) + ',full_space=' + str(
        self.full_space) + ',segs=' + str(self.segs) + ',dirty=' + str(
          self.dirty) + '>'
...
def validate_placement(board, bar, iseg, placement):
  ...
  elif placement + seg.length < len(board[bar.axis][bar.index]) and board[
      bar.axis][bar.index][placement + seg.length] == 1:
    return False
...
def check_bar(board, bar, start=0, iseg=0, tmp_placements=[], flags=[]):
  if iseg == len(bar.segs):
    ...
    for placement in seg.placements:
      ...
      check_bar(board,
                bar,
                start=placement + seg.length + 1,
                iseg=iseg + 1,
                tmp_placements=tmp_placements,
                flags=flags)
